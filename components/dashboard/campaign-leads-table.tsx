...
import type { CampaignLeadRow, PaginatedResult } from "@/lib/data-sources/types";
...
export function CampaignLeadsTable({
  ...
  result: PaginatedResult<CampaignLeadRow>;
  basePath: string;
  ...
  if (rows.length === 0) {
    return (
      <div className="flex h-40 items-center justify-center rounded-lg border text-sm text-muted-foreground">
        No unenrolled leads match these filters.
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-3">
      <div className="overflow-x-auto rounded-lg border">
        <Table>
          ...
          <TableBody>
            {rows.map((r) => (
              <TableRow key={r.formSubmissionId}>
